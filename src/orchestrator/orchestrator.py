...
MANUAL = (
    ...
    "        the steps to be executed. Re-run with confirm=True only after the\n"
    "        user approves; the pending plan is reused, not re-planned.\n"
    ...
    "      terms (list[str], optional): explicit search terms.\n"
    "    Returns: a validated JSON plan of steps, or 'aborted' when discovery\n"
    "      was poor.\n"
    "  - execute\n"
    "    Params:\n"
...
class Orchestrator:
    ...
    def __init__(
        ...
        *,
        ...
        self.executor = Executor(registry, self.undo_log, memory)
    ...
    def _awaiting_confirmation(self, steps: list[dict]) -> dict[str, Any]:
        summary = [
            {
                "step": i + 1,
                "tool": step.get("tool"),
    ...
    def run(
        ...
        if not confirm:
            return self._awaiting_confirmation(plan.get("steps", []))
