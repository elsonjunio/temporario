...
from src.providers.lmstudio import (
    LMStudioProvider,
    ProviderError as LMStudioProviderError,
)
...
def main() -> None:
    ...
    if (isinstance(provider, OpenCodeProvider) and not provider.api_key) or (
        isinstance(provider, LMStudioProvider) and not provider.model
    ):
        ui.show_info(
    ...
    while True:
        ...
        try:
            ...
            continue
        except (OpenCodeProviderError, LMStudioProviderError) as exc:
